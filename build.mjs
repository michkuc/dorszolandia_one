...
const BUILD_VERSION = 'v24.2-premium-shop-products-production';
const FILE_ID = '1RiXybg-8NtLTiq5oyAsVrK8fujzHT62Q';
const DRIVE_URL = `https://drive.google.com/uc?export=download&id=${FILE_ID}`;
const EXPECTED_SHA256 = '4b900b05fee126eb5c80e01cf42556eaed11bef64415ec60eb600e14fb047457';
const EXPECTED_BYTES = 12154643;
...
if (packed.length !== EXPECTED_BYTES) throw new Error(`Rozmiar v24.2 nie zgadza się: ${packed.length} != ${EXPECTED_BYTES}`);
if (sha !== EXPECTED_SHA256) throw new Error(`SHA256 v24.2 nie zgadza się: ${sha}`);
...
if (!shopHtml.includes('v24.2 premium shop products')) throw new Error('sklep.html nie ma markera v24.2 premium shop products.');
if (!shopHtml.includes('shop-reference-products')) throw new Error('Brak katalogu produktów z referencyjnej makiety v24.2.');
const productAssets = [
  ...
  'plakat-neptunopol.webp'
];
for (const asset of productAssets) if (!fs.existsSync(path.join('dist','assets','shop',asset))) throw new Error(`Brak produktu ${asset}`);
...
fs.writeFileSync('dist/vercel-build.txt', [
  ...
  `Premium shop products now match the user supplied reference artwork`,
  `Shop remains plan-only: no checkout or payment processing`,
